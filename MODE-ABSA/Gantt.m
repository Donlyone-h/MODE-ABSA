function Gantt(bpChorm,bsChorm,Tm,Trm,pNumber,jNumber,maxBatch,bbSize)

bpIChorm = bpChorm;
bsIChorm = bsChorm;
bsIndex = CtoI(bpChorm,bsChorm,Tm,maxBatch,jNumber);
% %根据设定的分批策略 得到分批结果
% [~,bbSize] = Cal(bpIChorm,bsIChorm,bsIndex,Tm,Trm,jNumber,pNumber,maxBatch);
%计算子代各目标适应度值
    ...
    if bIndex(bpIChorm(i)) ~= 0
        ...
        if pIndex(bpIChorm(i)) == 1 %第一道工序
            ...
            if pTable(bpIChorm(i)) > sTable(bsIndex(i))
                ...
                rec(4)=0.6;%矩形高度
                rectangle('Position',rec,'LineWidth',0.5,'LineStyle','-','FaceColor','#BEBEBE');
            else
                t = sTable(bsIndex(i));
            end
            %记录本工序时间
